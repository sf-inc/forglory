...
public class NoMixinFeats {
    ...
    public static void dashFeat(final PlayerEntity playerEntity) {
        ...
        double x = velocity.x + (((ModConfig.get().feats.dashIntensity + 2) / 4.0D) * Math.sin((-playerEntity.getYaw() * Math.PI) / 180.0D));
        double y = velocity.y + (((ModConfig.get().feats.dashIntensity + 2) / 6.0D) * Math.sin((-playerEntity.getPitch() * Math.PI) / 180.0D));
        double z = velocity.z + (((ModConfig.get().feats.dashIntensity + 2) / 4.0D) * Math.cos((-playerEntity.getYaw() * Math.PI) / 180.0D));
    ...
    public static void mountainFeat(final PlayerEntity playerEntity) {
        ...
        int height = ModConfig.get().feats.mountain.height;
