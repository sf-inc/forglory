...
public class NoMixinFeats {
    ...
    public static void dashFeat (final PlayerEntity playerEntity) {
        ...
        double x = velocity.x + ( ((ModConfig.get().dash_intensity+2) / 4.0D) * Math.sin((-playerEntity.yaw * Math.PI) / 180.0D));
        double y = velocity.y + ( ((ModConfig.get().dash_intensity+2) / 6.0D) * Math.sin((-playerEntity.pitch * Math.PI) / 180.0D));
        double z = velocity.z + ( ((ModConfig.get().dash_intensity+2) / 4.0D) * Math.cos((-playerEntity.yaw * Math.PI) / 180.0D));
    ...
    public static void mountainFeat(final PlayerEntity playerEntity) {
        ...
        int height = ModConfig.get().mountain_height;
